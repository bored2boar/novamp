...
export function clusterToCsv(cluster: Cluster): string {
  ...
  for (const member of cluster.members) {
    ...
    rows.push([
      ...
      record.flow?.complete ? record.flow.quoteInWei : "",
      record.flow?.complete ? record.flow.medianTicketWei : "",
      record.flow?.complete ? record.flow.top3Share.toFixed(4) : "",
      record.sellActivity?.complete ? (record.sellActivity.deployerSoldAtSec ?? "") : "",
      record.sellActivity?.complete ? (record.sellActivity.firstBigSellSec ?? "") : "",
      record.sellActivity?.complete ? record.sellActivity.largestShare.toFixed(4) : "",
      record.sellActivity?.complete ? record.sellActivity.sellBuyRatio.toFixed(4) : "",
      member.smartWallets,
      member.converged,

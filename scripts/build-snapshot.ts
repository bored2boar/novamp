...
query Meta($tokens: [String!]) {
  EVM(network: robinhood, dataset: realtime) {
    Transfers(
      limit: {count: 1500}
      where: {Transfer: {Currency: {SmartContract: {in: $tokens}}}}
    ) {
      Transfer { Currency { Name Symbol Decimals SmartContract } }
    }
  }
}`;
...
const META_CHUNK = Number(process.env["SNAPSHOT_META_CHUNK"] || 25);
const META_GAP_MS = Number(process.env["SNAPSHOT_META_GAP_MS"] || 400);
 ...
 * same indexer that gave us the launches. Asked in chunks with `in`, not one at
 * a time: a few requests instead of hundreds keeps a rate limited free key from
 * dropping most of the batch. A token that cannot be named is left out of the
 * map and lands in its own cluster - visible, not invented.
 */
async function fetchCurrencyMetaBatch(
  ...
  let done = 0;
  for (let i = 0; i < uniq.length; i += META_CHUNK) {
    const chunk = uniq.slice(i, i + META_CHUNK);
    try {
      ...
      }>(cfg, CURRENCY_QUERY, { tokens: chunk });
      for (const row of raw.EVM?.Transfers ?? []) {
        const cur = row.Transfer?.Currency;
        const addr = cur?.SmartContract?.toLowerCase();
      ...
      // whole chunk failed; those tokens stay unnamed, visible not invented
    }
    done += chunk.length;
    onProgress?.(Math.min(done, uniq.length), uniq.length);
    if (i + META_CHUNK < uniq.length) await metaSleep(META_GAP_MS);
  }
  return out;
}
